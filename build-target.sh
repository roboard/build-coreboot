...
# load TARGET_NAME from .target file.
if ! [ -f .target ]; then
  echo "Missing .target file, run set-target.sh first."
  exit 1
fi
...
# check coreboot/SeaBIOS directory
if ! [ -d ${BASE_DIR}/coreboot ]; then
  echo "Missing coreboot directory."
  exit 1
fi

if ! [ -d ${BASE_DIR}/seabios ]; then
  echo "Missing seabios directory."
  exit 1
fi
...
OUTPUT_NAME=`basename $TARGET_NAME`
...
(
  ...
  if ! [ -z $PAYLOAD_ELF ]; then
    ...
    cbfs_add_payload $PAYLOAD_ELF -n ${CONFIG_CBFS_PREFIX}/payload -c none
  fi
  ...
  zip -j out/$OUTPUT_NAME.zip coreboot/build/coreboot.rom
)
